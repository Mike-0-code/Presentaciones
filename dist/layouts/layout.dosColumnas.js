...
export class LayoutDosColumnas {
    ...
    renderizar(contenido) {
        ...
        const puntosIzqHTML = columnaIzquierda.puntos.map(punto => `<li>${this.escapeHTML(punto)}</li>`).join('');
        // Generar lista de puntos derecha
        const puntosDerHTML = columnaDerecha.puntos.map(punto => `<li>${this.escapeHTML(punto)}</li>`).join('');
        return `
